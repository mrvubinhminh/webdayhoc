...
const ModelIframeWrapper = () => {
    ...
    const url = `/models/${grade}/${encodeURIComponent(id)}.html`;
    ...
    return (
        <div ref={wrapperRef} className="w-full h-screen bg-[#111] flex flex-col">
            <header className="h-[60px] bg-gray-900 flex items-center justify-between px-4 border-b border-gray-800 shrink-0">
                ...
                >
                    <ArrowLeft className="w-5 h-5" /> Về Kho Mô Hình
                </button>
                <div className="flex gap-4 items-center">
                    <span className="text-emerald-500 font-bold hidden sm:inline">Mô Hình: {id.replace(/-/g, ' ').toUpperCase()}</span>
                    <button 
...
export default ModelIframeWrapper;
