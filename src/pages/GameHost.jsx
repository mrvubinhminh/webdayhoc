...
const HOST_ROOM_KEY = 'gameHostRoom';
...
const GameHost = () => {
  ...
  useEffect(() => {
    let timer;
    if (!roomData?.paused && (roomData?.status === 'QUESTION' || roomData?.status === 'REVEAL' || roomData?.status === 'STAR_PICK') && timeLeft > 0) {
      timer = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) {
             clearInterval(timer);
  ...
  const togglePause = async () => {
    if (!roomCode) return;
    await update(ref(db, `rooms/${roomCode}`), { paused: !roomData?.paused });
  };
  ...
  useEffect(() => {
    ...
    update(ref(db, `rooms/${roomCode}`), { scanRequestReveal: false });
    revealAnswer();
